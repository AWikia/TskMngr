...
if ((getComputedStyle(GetActiveThemeConfiguration()).getPropertyValue("--canvas-secondary-background-color") == 'auto') ) {
	...
	if (lightDropdown) {
		...
		var tabsoPage = window.MW18TabsOpacity + (chroma(dropdowncolor).luminance() * 0.4 );
	} else {
		var lightnessDropdown = '#ffffff';
		var tabscPage = '58,58,58';
		var tabsoPage = window.MW18TabsOpacity + ((1 - chroma(dropdowncolor).luminance()) * 0.4 );
	}
...
// Liatch Quirk
		...
		if (lightCaret) {
			...
			var tabsoCaret = window.MW18TabsOpacity + (chroma(caret_color).luminance() * 0.4 );
		} else {
			var lightnessCaret = '#ffffff';
			var tabscCaret = '58,58,58';
			var tabsoCaret = window.MW18TabsOpacity + ((1 - chroma(caret_color).luminance()) * 0.4 );
		}
...
if ((getComputedStyle(GetActiveThemeConfiguration()).getPropertyValue("--inactive-title-background-color") != 'auto') ) { // Only run Liatch quirk if not in autocolorization
	...
	if (lightCaretIT) {
		...
		var tabsoCaretIT = window.MW18TabsOpacity + (chroma(caretIT_color).luminance() * 0.4 );
	} else {
		var lightnessCaretIT = '#ffffff';
		var tabscCaretIT = '58,58,58';
		var tabsoCaretIT = window.MW18TabsOpacity + ((1 - chroma(caretIT_color).luminance()) * 0.4 );
	}
...
var link4color1 = ColorHover(link4_color,content_color);
	...
	if (lightLink) {
		...
		var tabsoLink = window.MW18TabsOpacity + (chroma(link_color).luminance() * 0.4 );
	} else {
		var tabscLink = '58,58,58';
		var tabsoLink = window.MW18TabsOpacity + ((1 - chroma(link_color).luminance()) * 0.4 );
	}
